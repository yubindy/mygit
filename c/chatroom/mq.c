...
{   
    char s[50];
    sprintf(s,"%s请求加好友，是否同意?(y.同意n.不同意)",recv_pack->send_name);
    recv_pack;
    node *t = head;
    while (t->next != NULL)
    {   
        t = t->next;
        if(strcmp(t.t->send_name,recv_pack->recv_name)==0)
        {
            send_t()
        }
    }
        ...
        for (int i = 0; i < nfs; i++)
        {
            ...
            else if (ep_ids[i].events & EPOLLIN) //读数据
            {
                ...
                else if (tsize == 0) //对端客户端关闭
                {
                    ...
                    if (packs->status == 1) //如果该用户上线，从上线链表中移除
                    {
                        node *r;
                        for (node *q = head; q = q->next; q->next != end)
                        {
                            if (q->next->t == packs)
                            {
                                pthread_mutex_lock(&lock);
